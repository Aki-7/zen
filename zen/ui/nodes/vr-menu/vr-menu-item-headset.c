#include "zen/ui/nodes/vr-menu/vr-menu-item-headset.h"
...
#include "zen/ui/nodes/vr-menu/vr-menu-headset-connect-button.h"
...
zn_vr_menu_item_headset_render(struct zigzag_node *node, cairo_t *cr)
{
  ...
  // TODO: Use the surface
  // cairo_set_source_surface(cr, headset->vr_icon_surface, icon_x, icon_y);
  // cairo_paint(cr);
  zigzag_cairo_stamp_svg_on_surface(
      cr, VR_ICON_TRANSLUCENT, icon_x, icon_y, vr_icon_width, vr_icon_height);
  ...
  zigzag_cairo_draw_text(cr, hmd_name, icon_x * 2 + vr_icon_width,
